...
for(const ptype of ['change_permission']){
  for(const target of ['permission_inheritance','user_permission_template','group_permission_template','user_self_permission_template']){
    permissions.push(`${ptype}.${target}`)
  }
}
for(const ttype of ['tier']){
  for(const action of ['create','delete','change_name','change_template','user_add','user_remove','group_add','group_remove']){
